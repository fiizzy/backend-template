...
const getRoleAccess = ({
  ...
  let userRole = await userRoleDb.findAll({ userId: userId },{ pagination:false });
  let routeRole = await routeRoleDb.findAll({ roleId: { $in: userRole ? userRole.map(u=>u.roleId) : [] } },{
    include: [{
      model: models.routeRole,
      as: 'routeId' 
    },{
      model:models.role,
      as:'roleId'
